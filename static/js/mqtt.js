...
$(document).ready(function() {
    function updateSubscribedTopics() {
        const subscribedTopicsSpan = $('#subscribedTopics');
        const allTopicsList = $('#all-topics');
        subscribedTopicsSpan.text(
            subscribedTopics.length > 0 ? subscribedTopics.join(", ") : "No subscriptions yet"
        );
        allTopicsList.empty(); // Clear old list
        subscribedTopics.forEach(topic => {
            allTopicsList.append(`<li>${topic}</li>`); // Add each topic as an <li>
        });
    }
