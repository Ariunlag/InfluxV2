...
def dashboard(section):
    ...
    try:
        if section == 'mqtt':
            ...
            #Class query and mqtt real time data logic here
            
            return render_template('dashboard.html', section='influx-mqtt', mqtt_connected=True, influx_connected=True)
...
def query():
    ...
    result = query_manager.start_query(section_id, measurements, time_range, aggregation, influx_client, socketio)
...
@app.route("/api/realtime/<query_name>", methods=["POST"])
def run_realtime(query_name):
    data = request.json
    section_id = data.get("section_id", "default")
    time_range = data.get('timeRange', None)
    aggregation = data.get('aggregation', None)
    ...
    try:
        ...
        result = influx_client.query_data( measurements, time_range, aggregation)
        socketio.emit(f"dataUpdate_{section_id}", result)
        if "error" in result:
            return jsonify(result), 404
        return jsonify(result), 200
